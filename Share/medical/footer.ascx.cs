...
public partial class Share_medical_footer : System.Web.UI.UserControl
{
    ...
    private void getBaiMoi()
    {
        ...
        string sqlCMD = "select top 3 * from news where isActived = 1 order by id desc ";
        DataTable dt = _db.sqlGetData(sqlCMD);
        rpLast.DataSource = dt;
        rpLast.DataBind();
    }
    protected void btnDH_Click(object sender, EventArgs e)
    {
        ...
        {
            DBClass _db = new DBClass();
            string pageName = "Phòng khám sản khoa", address = "", name = txtFullname.Text, email = "", phone = txtSDT.Text, content = txtMsg.Text, domain = "phongkhamsankhoa.com";
